...
const PortfolioSection: React.FC<PortfolioSectionProps> = ({ isAiMode, onOpenCaseStudy }) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const filtered = isAiMode
        ? PROJECTS.filter(p => p.category === ProjectCategory.AI_ML)
        : PROJECTS.filter(p => p.category !== ProjectCategory.AI_ML);
  ...
  const handleFilterChange = (category: string) => {
    ...
    setTimeout(() => {
      const baseProjects = isAiMode
        ? PROJECTS.filter(p => p.category === ProjectCategory.AI_ML)
        : PROJECTS.filter(p => p.category !== ProjectCategory.AI_ML);

      const newProjects = category === 'ALL' 
        ? baseProjects 
        : baseProjects.filter(p => p.category === category);
  ...
  const categories = isAiMode 
    ? [{ label: 'All AI Projects', value: 'ALL' }, { label: 'Machine Learning', value: ProjectCategory.AI_ML }]
    : [
        ...
        <div className="flex flex-col lg:flex-row lg:justify-between lg:items-end mb-12 sm:mb-20 gap-8">
          <div className="max-w-xl">
            <h2 className="text-3xl sm:text-5xl md:text-6xl font-serif font-bold mb-4 sm:mb-6 text-gray-900 dark:text-white leading-tight">
              {isAiMode ? 'Intelligence in Code' : 'Selected Works'}
            </h2>
            <p className="text-gray-500 dark:text-gray-400 text-base sm:text-lg">
              {isAiMode 
                ? "Exploring the frontiers of Deep Learning, NLP, and Computer Vision architectures."
                : "A fusion of strategic visual storytelling and pixel-perfect digital interaction."
              }
            </p>
          </div>
